...
def _open_gpkg(path):
    ...
    ds = ogr.Open(path)
    if ds is None:
        raise ValueError(f"GeoPackage non leggibile / unreadable GeoPackage: {path}")
    return ds


def _read_layer_records(ds, layer_name, fields):
    """Read *fields* from every feature of *layer_name*; missing fields become None."""
    layer = ds.GetLayerByName(layer_name)
    if layer is None:
        return []
    defn = layer.GetLayerDefn()
    available = {defn.GetFieldDefn(i).GetName() for i in range(defn.GetFieldCount())}
    records = []
    layer.ResetReading()
    for feature in layer:
        rec = {}
        for field in fields:
            rec[field] = feature.GetField(field) if field in available else None
        records.append(rec)
    return records
...
def read_gpkg_summary(path):
    ...
    for i in range(ds.GetLayerCount()):
        ...
        if low.startswith(_PREFIX_SECTIONS_CENTERS):
            summary["sections"] = _read_layer_records(ds, name, [
                "sezione", "prog_m", "quota_min", "quota_max", "quota_prj",
                "area_m2", "cut_m2", "fill_m2",
            ])
        elif low.startswith(_PREFIX_VOLUMES):
            summary["volumes"] = _read_layer_records(ds, name, [
                "da_sez", "a_sez", "dist_m", "volume_m3",
                "sterro_m3", "riporto_m3", "cum_sterro", "cum_riporto",
            ])
        elif low.startswith(_PREFIX_PROFILE_SAMPLES):
            summary["samples"] = _read_layer_records(ds, name, [
                "prog_m", "quota_m",
            ])
        elif low.startswith(_PREFIX_PROFILE_PICKETS):
            summary["pickets"] = _read_layer_records(ds, name, [
                "id", "prog_m", "quota_m", "label",
            ])
    ds = None
    ...
    *tolerance_m*. Profiles are compared by linear interpolation of the
    ...
    if before["volumes"] or after["volumes"]:
        ...
        result["volume_totals"] = {
            "cut_before": cut_b, "fill_before": fill_b,
            "cut_after": cut_a, "fill_after": fill_a,
            "d_cut": cut_a - cut_b, "d_fill": fill_a - fill_b,
    ...
    if prof_b and prof_a:
        ...
        for prog, z_b in prof_b:
            ...
            if z_a is None:
                continue
            deltas.append({"prog_m": prog, "z_before": z_b, "z_after": z_a, "dz": z_a - z_b})
...
def generate_delta_svg(deltas, lang="it", width=1700, height=420):
    ...
    zero_y = py(0)
    path_pts = " ".join(f"{px(d['prog_m']):.1f},{py(d['dz']):.1f}" for d in deltas)
    ...
    title = _t(lang, "Variazione quota (dopo - prima)", "Elevation change (after - before)")
  ...
  <line x1="{margin_l}" y1="{zero_y:.1f}" x2="{margin_l + plot_w}" y2="{zero_y:.1f}"
        stroke="#8ba3c7" stroke-width="1.2" stroke-dasharray="6,4"/>
  <polyline points="{path_pts}" fill="none" stroke="#4f73c4" stroke-width="1.8"/>
...
def generate_compare_html(result, lang="it", chart_html=""):
    ...
    main_title = _t(L, "Confronto prima / dopo lavori", "Before / after works comparison")
    parts.append(f"<div class='section-title'>{main_title}</div>")
    parts.append(
        "<div class='summary-card'>"
        f"<strong>{_t(L, 'Prima / Before', 'Before / Prima')}:</strong> {before['name']} "
        f"<span class='badge badge-info'>{before['kind']}</span><br>"
        f"<strong>{_t(L, 'Dopo / After', 'After / Dopo')}:</strong> {after['name']} "
        f"<span class='badge badge-info'>{after['kind']}</span><br>"
        f"{_t(L, 'Tolleranza accoppiamento sezioni', 'Section matching tolerance')}: "
    ...
    if stats:
        sign_note = _t(
            L,
            "Δ positivo = terreno più alto dopo i lavori (riporto); Δ negativo = più basso (scavo).",
            "Positive Δ = ground higher after works (fill); negative Δ = lower (cut).",
        )
        parts.append(
            "<div class='summary-card'>"
            f"<strong>{_t(L, 'Variazione quote sul profilo', 'Profile elevation change')}</strong><br>"
            f"{_t(L, 'Punti confrontati', 'Compared stations')}: <strong>{stats['n']}</strong> &nbsp;·&nbsp; "
            f"Δ {_t(L, 'media', 'mean')}: <strong>{stats['mean']:+.3f} m</strong> &nbsp;·&nbsp; "
    ...
    if totals:
        ...
        parts.append(
            ...
            f"<tr><td><span class='badge badge-cut'>{_t(L, 'Sterro', 'Cut')}</span></td>"
            f"<td>{_fmt(totals['cut_before'])} m³</td><td>{_fmt(totals['cut_after'])} m³</td>"
            f"<td><strong>{totals['d_cut']:+,.2f} m³</strong></td></tr>"
            f"<tr><td><span class='badge badge-fill'>{_t(L, 'Riporto', 'Fill')}</span></td>"
            f"<td>{_fmt(totals['fill_before'])} m³</td><td>{_fmt(totals['fill_after'])} m³</td>"
            f"<td><strong>{totals['d_fill']:+,.2f} m³</strong></td></tr>"
            f"<tr><td>{_t(L, 'Bilancio (sterro - riporto)', 'Balance (cut - fill)')}</td>"
    ...
    if pairs:
        ...
        rows = [
            "<table><tr>"
            f"<th>{_t(L, 'Sez.', 'Sec.')}</th><th>{_t(L, 'Prog. (m)', 'Chainage (m)')}</th>"
        ...
        if result["unmatched_before"] or result["unmatched_after"]:
            unmatched_msg = _t(
                L,
                f"Sezioni non accoppiate — prima: {result['unmatched_before']}, "
    ...
    if not stats and not totals and not pairs:
        no_data_msg = _t(
            L,
            "Nessun dato confrontabile trovato nei due GeoPackage. Servono layer generati "
            "da questo plugin (campioni profilo, picchetti, centri sezione o tratte volumi).",
            "No comparable data found in the two GeoPackages. Layers generated by this plugin "
            "are required (profile samples, pickets, section centers or volume segments).",
        )
...
def compare_dtm_rasters(path_before, path_after, output_path,
                        threshold_m=0.05, progress=None):
    ...
    if ds_before is None:
        raise ValueError(f"Raster non leggibile / unreadable raster: {path_before}")
    ds_after = gdal.Open(path_after)
    if ds_after is None:
        raise ValueError(f"Raster non leggibile / unreadable raster: {path_after}")

    gt = ds_before.GetGeoTransform()
    proj = ds_before.GetProjection()
    if not proj:
        raise ValueError("Il raster 'prima' non ha un CRS definito / 'before' raster has no CRS.")
    ...
    transform = None
    if proj_after and not srs_before.IsSame(osr.SpatialReference(wkt=proj_after)):
    ...
    if ix_min >= ix_max or iy_min >= iy_max:
        raise ValueError(
            "I due raster non si sovrappongono / the two rasters do not overlap."
        )
    ...
    if geographic:
        ...
        pixel_area_m2 = (cell_w * 111320.0 * math.cos(math.radians(lat_center))) * (cell_h * 110540.0)
    else:
    ...
    stats = {
        "n_valid": 0, "sum_dz": 0.0, "sum_sq": 0.0,
        "min": None, "max": None,
        "vol_fill_m3": 0.0, "vol_cut_m3": 0.0,
        "n_fill": 0, "n_cut": 0,
    }
    ...
    for row0 in range(0, rows, block_rows):
        ...
        if valid.size:
            ...
            stats["min"] = v_min if stats["min"] is None else min(stats["min"], v_min)
            stats["max"] = v_max if stats["max"] is None else max(stats["max"], v_max)
        ...
        _report(20 + int(70 * (row0 + nrows) / rows), "Calcolo differenza / Computing difference")
...
def generate_dtm_compare_html(result, path_before, path_after, lang="it"):
    L = lang
    parts = []
    dtm_title = _t(L, "Confronto DTM (differenza dopo - prima)", "DTM comparison (after - before difference)")
    parts.append(f"<div class='section-title'>{dtm_title}</div>")
    parts.append(
        "<div class='summary-card'>"
        f"<strong>{_t(L, 'Prima', 'Before')}:</strong> {os.path.basename(path_before)}<br>"
        f"<strong>{_t(L, 'Dopo', 'After')}:</strong> {os.path.basename(path_after)}<br>"
        f"<strong>{_t(L, 'Raster differenza', 'Difference raster')}:</strong> {result['diff_path']}<br>"
        f"{_t(L, 'Celle valide', 'Valid cells')}: {result['n_valid']:,} "
        f"({result['cols']} × {result['rows']}) &nbsp;·&nbsp; "
        f"{_t(L, 'Soglia variazione', 'Change threshold')}: ±{result['threshold_m']:.2f} m"
        "</div>"
    )
    ...
    if result.get("geographic_approx"):
        crs_warning = _t(
            L,
            "Attenzione: CRS geografico — aree e volumi sono approssimati. Per valori "
            "accurati usare DTM in CRS metrico (es. UTM).",
            "Warning: geographic CRS — areas and volumes are approximate. For accurate "
            "figures use DTMs in a metric CRS (e.g. UTM).",
        )
        parts.append(f"<div class='summary-card' style='color:#f5a623'>{crs_warning}</div>")
    style_note = _t(
        L,
        "Il raster differenza è caricato in QGIS con scala di colori divergente: "
        ...
        "red = lowering (cut), blue = raising (fill). Deterministic GDAL/NumPy "
        "computation, no AI.",
    )
