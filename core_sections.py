...
from qgis.core import (
    QgsProject, QgsRaster, QgsPointXY,
...
def calculate_cross_sections(
        line_points, raster_layer, interval_m, half_width_m,
        samples_per_section, design_start_elev, design_end_elev,
        design_grade_pct, smooth_tangent, band=1):
    """
    ...
    try:
        xform_to_wgs = QgsCoordinateTransform(
            QgsProject.instance().crs(), crs_wgs,
            QgsProject.instance().transformContext()
        )
        xform_from_wgs = QgsCoordinateTransform(
            crs_wgs, raster_layer.crs(),
            QgsProject.instance().transformContext()
        )
    except Exception:
        xform_to_wgs = QgsCoordinateTransform(
    ...
    for a, b in zip(pts_wgs[:-1], pts_wgs[1:]):
        ...
        if dist > 0:
            segs.append({"a": a, "b": b, "d0": total_len, "d1": total_len + dist, "az": _az1})
            total_len += dist
    ...
    def _point_at(dist_m):
        ...
        for i, s in enumerate(segs):
            if dist_m <= s["d1"] or i == len(segs) - 1:
                ...
                break
        s = segs[si]
        frac = 0.0 if s["d1"] <= s["d0"] else (dist_m - s["d0"]) / (s["d1"] - s["d0"])
    ...
    for i in range(n_sections):
        ...
        for off in offsets:
            ...
            sec_pts.append({"offset_m": round(off, 2), "elevation": z, "lon": plon, "lat": plat})

        valid_pts = [(p["offset_m"], p["elevation"]) for p in sec_pts if p["elevation"] is not None]
        ...
        if len(valid_pts) >= 2:
            for k in range(len(valid_pts) - 1):
                ...
                area_total += abs(dx) * ((valid_pts[k][1] + valid_pts[k + 1][1]) / 2 - (min_z or 0))
        ...
        sections.append({
            ...
            "design_elevation": round(design_elev, 3) if design_elev is not None else None,
    ...
    road_metrics = {}
    if grades_pct:
        road_metrics["avg_grade_pct"] = round(sum(abs(g) for g in grades_pct) / len(grades_pct), 3)
        road_metrics["max_grade_pct"] = round(max((abs(g) for g in grades_pct), default=0), 3)
        road_metrics["min_grade_pct"] = round(min((abs(g) for g in grades_pct), default=0), 3)
    ...
    if curve_radii:
        ...
        road_metrics["avg_radius_m"] = round(sum(curve_radii) / len(curve_radii), 1)
    ...
    for i in range(1, len(pts_wgs) - 1):
        ...
        radius = None
        try:
            chord = (prev_seg["d1"] - prev_seg["d0"] + next_seg["d1"] - next_seg["d0"]) / 2.0
        ...
        curve_points.append({
            ...
            "estimated_radius_m": round(radius, 3) if radius is not None else None,
        })
    ...
    return {
        ...
        "design_profile": {
            ...
            "grade_pct": round(design_grade, 3) if design_grade is not None else None,
        },
...
def generate_cross_sections_svg(result, project_title, created_at):
    ...
    valid_sections = [
        s for s in sections
        if s.get("min_elevation") is not None and s.get("max_elevation") is not None
    ]
    ...
    long_svg = '<text x="90" y="255" class="muted">Insufficient elevation data for longitudinal profile.</text>'
    ...
    if len(valid_sections) >= 2:
        ...
        def x_of(d):
            return chart_l + ((float(d) - min_x) / span_x) * (chart_r - chart_l)

        def y_of(z):
            return chart_b - ((float(z) - min_y) / span_y) * (chart_b - chart_t)

        line_d = " ".join(
            f"{'M' if i == 0 else 'L'}{x_of(s.get('progressive_m')):.1f},{y_of(elevs[i]):.1f}"
        ...
        for c in curves:
            ...
            curve_marks.append(
                f'<line x1="{x:.1f}" y1="{chart_t}" x2="{x:.1f}" y2="{chart_b}" class="curve-line"/>'
                f'<circle cx="{x:.1f}" cy="{chart_t + 12}" r="7" class="curve-dot"/>'
                f'<text x="{x:.1f}" y="{chart_t - 8}" class="curve-label">C{esc(c.get("index"))}</text>'
            )
        axis_labels = []
        for t in (0, 0.25, 0.5, 0.75, 1):
            ...
            x = x_of(d)
            axis_labels.append(
                f'<text x="{x:.1f}" y="{chart_b + 28}" class="axis center">{d:.0f} m</text>'
            )
        long_svg = (
            ...
            f'Longitudinal Profile with Cross Sections / Profilo Longitudinale con Sezioni</text>'
        )
    ...
    for idx, sec in enumerate(sections[:18]):
        ...
        if len(pts) < 2:
            thumbs.append(
                f'<text x="{x0}" y="{y0 + 45}" class="muted">Sec. {esc(sec.get("index"))}: no elev.</text>'
            )
            continue
        ...
        def ty(z):
            return y0 + thumb_h - 28 - ((float(z) - mn_y) / sp_y) * (thumb_h - 46)

        d = " ".join(
            f"{'M' if i == 0 else 'L'}{tx(p.get('offset_m')):.1f},{ty(p.get('elevation')):.1f}"
        ...
        thumbs.append(
            f'<g><rect x="{x0}" y="{y0}" width="{thumb_w}" height="{thumb_h}" class="thumb"/>'
            f'<path d="{d}" class="section-line"/>{ref}'
            f'<text x="{x0 + 12}" y="{y0 + 18}" class="thumb-label">'
            f'Sec. {esc(sec.get("index"))} · {float(sec.get("progressive_m") or 0):.0f} m</text>'
            f'<text x="{x0 + 12}" y="{y0 + thumb_h - 8}" class="thumb-meta">'
            f'A={float(sec.get("area_m2") or 0):.2f} m² · S={float(sec.get("cut_area_m2") or 0):.2f}'
    ...
    for i, v in enumerate(volumes[:4]):
        y = table_y + 28 + i * 22
        volume_rows.append(
            f'<text x="92" y="{y}" class="cell">{esc(v.get("from_section"))} → {esc(v.get("to_section"))}</text>'
            f'<text x="260" y="{y}" class="cell right">{float(v.get("distance_m") or 0):.1f}</text>'
            f'<text x="430" y="{y}" class="cell right">{float(v.get("volume_m3") or 0):.1f}</text>'
            f'<text x="610" y="{y}" class="cell right">{float(v.get("cut_m3") or 0):.1f}</text>'
            f'<text x="790" y="{y}" class="cell right">{float(v.get("fill_m3") or 0):.1f}</text>'
        )
...
<defs>
  <style>
    ...
    .section-title {{ font: 700 17px Arial, sans-serif; fill:#f1f5f9; }}
    .axis {{ font: 12px Arial, sans-serif; fill:#8ba3c7; }}
    .center {{ text-anchor:middle; }}
    .frame,.thumb {{ fill:#0e1118; stroke:#2d3757; stroke-width:1.2; }}
    .section {{ stroke:#4f73c4; stroke-width:1; stroke-dasharray:3 5; opacity:.75; }}
    .section-strong {{ stroke:#f97316; stroke-width:1.6; stroke-dasharray:5 4; }}
    .long-line {{ fill:none; stroke:#4f73c4; stroke-width:4; stroke-linecap:round; stroke-linejoin:round; }}
    .curve-line {{ stroke:#f97316; stroke-width:2; stroke-dasharray:6 4; }}
    .curve-dot {{ fill:#f97316; stroke:#111827; stroke-width:1.5; }}
    .curve-label {{ font:700 11px Arial, sans-serif; fill:#9a3412; text-anchor:middle; }}
    .section-line {{ fill:none; stroke:#4f73c4; stroke-width:2; }}
    .design-line {{ stroke:#f59e0b; stroke-width:1.5; stroke-dasharray:5 3; }}
    .thumb-label {{ font:700 12px Arial, sans-serif; fill:#e2e8f0; }}
    .thumb-meta,.muted {{ font:11px Arial, sans-serif; fill:#8ba3c7; }}
    .cell {{ font:12px Arial, sans-serif; fill:#e2e8f0; }}
    .right {{ text-anchor:end; }}
  </style>
</defs>
<rect width="100%" height="100%" fill="#12151e"/>
<text x="70" y="52" class="title">Cross Sections and Road Profile / Sezioni Trasversali</text>
<text x="70" y="80" class="subtitle">
  Project: {esc(project_title)} · Axis {float(result.get("alignment_length_m") or 0):.2f} m ·
  Sections {len(sections)} · Cut {float(result.get("total_cut_m3") or 0):.2f} m³ ·
  Fill {float(result.get("total_fill_m3") or 0):.2f} m³ · {esc(created_at)}
</text>
{long_svg}
<text x="80" y="470" class="section-title">Cross Sections / Sezioni Trasversali</text>
{"".join(thumbs)}
<text x="80" y="965" class="section-title">Volume Summary / Prime Tratte Volume</text>
...
def generate_cross_sections_html(data):
    ...
    <div style="background:#0f172a;border:1px solid #243244;color:#cbd5e1;padding:12px;
        ...
        <strong>Axis:</strong> {(data['alignment_length_m'] / 1000):.3f} km &nbsp;·&nbsp;
    ...
    if has_ref:
        ...
        <span style="color:#ef4444;">Cut: <strong>{data['total_cut_m3']:,.0f} m³</strong>
        </span>&nbsp;·&nbsp;
        <span style="color:#22c55e;">Fill: <strong>{data['total_fill_m3']:,.0f} m³</strong></span>&nbsp;·&nbsp;
        <span style="color:#f59e0b;">Balance:
        <strong>{(data['total_fill_m3'] - data['total_cut_m3']):,.0f} m³</strong></span>
        """
    html += "</div>"
    ...
    if m:
        ...
        c_text = (
            f"Curves: <strong>{c_count}</strong> · Min radius: <strong>{m.get('min_radius_m', 'n/a')} m</strong>"
            if c_count > 0 else "Straight alignment (no curves detected)"
        )
        html += f"""
        <div style="background:#0f172a;border:1px solid #243244;color:#cbd5e1;padding:12px;
                    border-radius:6px;margin-bottom:12px;font-size:13px;line-height:1.5;">
            <strong style="color:#d97706;">Road Metrics</strong><br>
            Avg grade: <strong>{m.get('avg_grade_pct', 'n/a')} %</strong> &nbsp;·&nbsp;
            Max: <strong>{m.get('max_grade_pct', 'n/a')} %</strong> &nbsp;·&nbsp;
            Min: <strong>{m.get('min_grade_pct', 'n/a')} %</strong><br>
            Total relief: <span style="color:#ef4444;">▲ {m.get('total_ascent_m', 0):.1f} m</span>
            &nbsp;<span style="color:#22c55e;">▼ {m.get('total_descent_m', 0):.1f} m</span><br>
            {c_text}
        </div>
        """
    ...
    for sec in data["sections"][:12]:
        ...
        if len(pts) < 2:
            svgs += (
                ...
                f'color:#64748b;width:120px;">Sec.{sec["index"]}<br>no data</div>'
            )
            continue
        ...
        d_path = " ".join([
            f"{'M' if i == 0 else 'L'}{xOf(pts[i]['offset_m']):.1f},{yOf(pts[i]['elevation']):.1f}"
        ...
        if has_ref and sec.get("design_elevation") is not None:
            try:
                ...
                ref_line = (
                    f'<line x1="{padX}" y1="{ry:.1f}" x2="{w - padX}" y2="{ry:.1f}"'
        ...
        <div style="flex-shrink:0;text-align:center;width:120px;">
            ...
            style="border:1px solid #334155;border-radius:5px;background:#111827;display:block;">
                <path d="{fill_d}" fill="rgba(77,159,255,0.15)" stroke="none"/>
                <path d="{d_path}" fill="none" stroke="#4d9fff" stroke-width="1.5"/>
                {ref_line}
            </svg>
    ...
    html += f'<div style="display:flex;flex-wrap:wrap;gap:8px;padding:0 0 10px 0;">{svgs}'
    ...
    if data.get("volumes"):
        ...
        for v in data["volumes"]:
            ...
            if None not in (s1d["min_elevation"], s2d["min_elevation"]) and v["distance_m"] > 0:
                ...
                grade_str = f'<td style="text-align:right;color:{col};">{g:.2f}%</td>'
            rows += f"""<tr>
            ...
            if has_ref:
                rows += (
                    f'<td style="text-align:right;color:#ef4444;padding:4px;">{v["cut_m3"]:,.1f}</td>'
                    f'<td style="text-align:right;color:#22c55e;padding:4px;">{v["fill_m3"]:,.1f}</td>'
                    f'<td style="text-align:right;color:#ef4444;padding:4px;">{v["cumulative_cut_m3"]:,.0f}</td>'
                    f'<td style="text-align:right;color:#22c55e;padding:4px;">{v["cumulative_fill_m3"]:,.0f}</td>'
                )
            rows += "</tr>"
        header_ref = (
            '<th style="color:#ef4444;border-bottom:1px solid #cbd5e1;">Cut (m³)</th>'
            '<th style="color:#22c55e;border-bottom:1px solid #cbd5e1;">Fill (m³)</th>'
            '<th style="color:#ef4444;border-bottom:1px solid #cbd5e1;">Cum.Cut</th>'
            '<th style="color:#22c55e;border-bottom:1px solid #cbd5e1;">Cum.Fill</th>'
        ) if has_ref else ""
        html += f"""
        <table style="width:100%;border-collapse:collapse;font-size:12px;
        background:#0f172a;color:#cbd5e1;border:1px solid #243244;">
            <thead><tr>
                <th style="padding:6px 4px;text-align:center;border-bottom:1px solid #cbd5e1;">Sections</th>
                <th style="padding:6px 4px;text-align:right;border-bottom:1px solid #cbd5e1;">Dist (m)</th>
                <th style="padding:6px 4px;text-align:right;border-bottom:1px solid #cbd5e1;">Grade %</th>
                <th style="padding:6px 4px;text-align:right;border-bottom:1px solid #cbd5e1;">A₁ (m²)</th>
                <th style="padding:6px 4px;text-align:right;border-bottom:1px solid #cbd5e1;">A₂ (m²)</th>
                <th style="padding:6px 4px;text-align:right;border-bottom:1px solid #cbd5e1;">Vol (m³)</th>
                {header_ref}
            </tr></thead>
            <tbody>{rows}</tbody>
        </table>
...
def generate_cross_sections_results_html(data):
    """Full HTML for the Results tab (summary + sections + volumes + detail table)."""
    dp = data.get("design_profile", {})
    has_ref = dp.get("start_elevation") is not None
    ...
    if has_ref:
        ...
        <span class="badge badge-cut">Cut: {data['total_cut_m3']:,.0f} m³</span> &nbsp;
        <span class="badge badge-fill">Fill: {data['total_fill_m3']:,.0f} m³</span> &nbsp;
        <span class="badge badge-info">Balance: {(data['total_fill_m3'] - data['total_cut_m3']):,.0f} m³</span>
        """
    ...
    if m:
        ...
        html += f"""
        <div class="summary-card">
            <strong style="color:#f59e0b;">Road Metrics / Metriche Stradali</strong><br>
            Avg grade: <strong>{m.get('avg_grade_pct', 'n/a')} %</strong> &nbsp;·&nbsp;
            Max: <strong>{m.get('max_grade_pct', 'n/a')} %</strong> &nbsp;·&nbsp;
            Min: <strong>{m.get('min_grade_pct', 'n/a')} %</strong><br>
            Relief: <span style="color:#ef4444;">▲ {m.get('total_ascent_m', 0):.1f} m</span>
            &nbsp;<span style="color:#22c55e;">▼ {m.get('total_descent_m', 0):.1f} m</span>
            &nbsp;·&nbsp; Curves: <strong>{c_count}</strong>
        """
        if c_count > 0:
            html += (
                f" &nbsp;·&nbsp; R min: <strong>{m.get('min_radius_m', 'n/a')} m</strong>"
                f" &nbsp;·&nbsp; R avg: <strong>{m.get('avg_radius_m', 'n/a')} m</strong>"
            )
        html += "</div>"
        ...
        for v in data["volumes"]:
            ...
            if has_ref:
                html += (
                    ...
                    f'<td style="color:#ef4444;">{v["cumulative_cut_m3"]:,.0f}</td>'
                    f'<td style="color:#22c55e;">{v["cumulative_fill_m3"]:,.0f}</td>'
                )
            html += "</tr>"
        html += "</tbody></table>"
    ...
    html += (
        ...
        "<table><thead><tr>"
        "<th>Sec.</th><th>Progressive (m)</th><th>Min (m)</th><th>Max (m)</th><th>Area (m²)</th>"
    )
    if has_ref:
    ...
    for s in data["sections"]:
        ...
        if has_ref:
            html += (
                f'<td style="color:#ef4444;">{s.get("cut_area_m2", 0):.2f}</td>'
                f'<td style="color:#22c55e;">{s.get("fill_area_m2", 0):.2f}</td>'
            )
        html += "</tr>"
    html += "</tbody></table>"
    return html
